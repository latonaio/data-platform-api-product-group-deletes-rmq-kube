...
import (
	...
	"data-platform-api-product-group-deletes-rmq-kube/config"
...
func (c *DPFMAPICaller) deleteSqlProcess(
	...
	var productGroupData *dpfm_api_output_formatter.ProductGroup
	for _, a := range accepter {
		switch a {
		case "ProductGroup":
			h := c.productGroupDelete(input, output, log)
			productGroupData = h
			if h == nil {
				continue
			}
...
func (c *DPFMAPICaller) productGroupDelete(
	input *dpfm_api_input_reader.SDC,
	output *dpfm_api_output_formatter.SDC,
	log *logger.Logger,
) *dpfm_api_output_formatter.ProductGroup {
	sessionID := input.RuntimeSessionID
	productGroup := c.ProductGroupRead(input, log)
	productGroup.IsMarkedForDeletion = input.ProductGroup.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": productGroup, "function": "ProductGroupProductGroup", "runtime_session_id": sessionID})
	if err != nil {
	...
	if !checkResult(res) {
		...
		output.SQLUpdateError = "ProductGroup Data cannot delete"
		return nil
	}
